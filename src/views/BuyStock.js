...
//import {schedulebuy} from "../views/TransactionStatus";
...
const BuyStock=()=>{
  ...
  //載入User
    ...
    async function getStockprice(stocknum){
        ...
        //let res1=await requests.get(`${cors}${url}`);
   ...
   async function getStockprice2(stocknum){
    ...
    function likestock(){
        ...
        push(ref(db,'/Userlike/'+account),{
            stock_name: stockdata.c,
            stock_price:stockdata.y,
            time:Date(),
        });
    }
    ...
    const can_buy_or_not=async(res,req)=>{
        get_now_money();
        ...
        try{
            let result= await contract_call(stock_contract,'User_Can_Buy_OR_NOT',[account,user_money,stockdata.c,price,y],{
              from:accounts[0],
              gas:6000000,
            });
            if (result) {
              ...
              let finalmoney;
              //console.log(success_or_not)
              console.log("!判斷餘額是否足夠"+y);
              //alert(result);
              let check=Object.values(result)[0];
              console.log(check);
              if(check === true){
                console.log('餘額足夠購買'+stockdata.c+'股票'+y+'股，開始執行購買股票');
                  console.log('餘額足夠購買'+stockdata.c+'股票'+y+'股');
                  ...
                  const buy_stock=async(res,req)=>{
                    try{
                        let buy_stock = await contract_send(stock_contract,'User_Buy',[account,stockdata.c,price,y],{
                          from:accounts[0],
                          gas:6000000,
                        });
                  ...
                  //搜尋是否持有該股票
                  const dbRef = ref(getDatabase());
                  const dbRef2 = child(dbRef,"Buy");
                  const dbRef3=query(dbRef2,orderByChild('user_account'),equalTo(''+account),equalTo(''+stockdata.c));
                  var newUserData = {
                    'stock_name': 'test123',
                    'stock_price': '233',
                  };
                  update(dbRef3,newUserData);
                */
                 ...
                 //檢查是否已持有該股票
                 console.log("檢查是否已持有該股票"+account+stockdata.c);
                  try{
                      const dbRef = ref(getDatabase());
                      const dbRef2 = child(dbRef,"Buy/"+account);
                      const dbRef3=query(dbRef2,orderByChild('stock_name'),equalTo(''+stockdata.c));
          
                              //orderByChild('stock_name'),equalTo(''+stockdata.c)
                             // where('user_account','==','7110026106')
                      get(dbRef3).then((snapshot) => {
                          if (snapshot.exists()){
                              ...
                              alert('市價單將在'+txn_time+'進行處理');
                              const job1 = (stock_name=stockdata.c,stock_number=y)=>{
                                  schedule.scheduleJob(txn_time, () => {
                                    console.log('開始執行交易'+ new Date());
                                    console.log(stock_name,stock_number);
                                    getStockprice(stock_name);
                                    console.log('現在價格'+stockdata)
                                  })
                              }
                              job1();
                              */
                ...
                console.log('餘額不足無法購買'+stockdata.c+'股票'+y+'股');
                console.log('餘額不足無法購買'+stockdata.c+'股票'+y+'股');
              }
              //Navigate.push('Home/Register');
    ...
    //將訂單存至等待購買執行佇列
    const waiting_buy_order=async(res,req)=>{
        var x = document.getElementById("TransactionType").value;
        var y = document.getElementById("share").value;
        ...
        if(x=='1'){
            ...
            await(schedulebuy(stockdata.c,y));
        }
        else if(x=='2'){ //限價交易
            console.log('限價交易');
            let price=parseFloat(stockdata.z);
    ...
    //取得喜好股票
    const getlikedata=async(res,req)=>{
        console.log("開始抓資料");
        const dbRef = ref(getDatabase());
        const dbRef2 = child(dbRef,"Userlike/"+account);
        get(dbRef2).then((snapshot) => {
            //child(dbRef, 'Buy/')
            if(snapshot.exists()) {
...
/*
 //取得現在購買數
 const getbuynum=async(res,req)=>{
    alert("開始取得資料庫購買數");
    const dbRef = ref(getDatabase());
    get(child(dbRef, 'HowmanyBuy/')).then((snapshot) => {
        if(snapshot.exists()) {
          //const alldata=snapshot.val()
          //alert('有抓到'+alldata);
          let buynum=snapshot.val().number;
          settest(buynum);
...
*/
    //股票是否已經存在
    const stock_exists=async(res,req)=>{
        console.log("股票是否存在"+account+stockdata.c);
        try{
            const dbRef = ref(getDatabase());
            const dbRef2 = child(dbRef,"Buy/"+account);
            const dbRef3=query(dbRef2,orderByChild('stock_name'),equalTo(''+stockdata.c));
            ...
            get(dbRef3).then((snapshot) => {
                if (snapshot.exists()){
                    ...
                    console.log(Object.values(userData)[0].stock_number);
                    let userhave=parseInt(Object.values(userData)[0].stock_number)+2;
                    console.log('userhave:'+userhave);
                    let userhave2=Object.keys(userData);
                    console.log('userhave2:'+userhave2);
    ...
    async function get_final_money(final){
        ...
        console.log('目前最終金額'+user_final_money);
        console.log('目前最終金額'+finalmoneynow);
        ...
        return (props.trigger)?(
            <div className="popup">
                ...
                <button className="close-btn" onClick={()=>{props.setButtonPop(false);window.location.reload()}}>close</button>
                        ...
                        <input class='inputshare' type="text" id='share' placeholder="輸入購買股數"/>
                        <input class='inputprice' type="text" id='wantprice' placeholder="限價交易購買價格"/>
                        <button class="cancel"  onClick={()=>{props.setButtonPop(false)}}>取消交易</button>
                        <button class="submit"  onClick={()=>{waiting_buy_order()}}>確認送出</button>
                ...
                try{
                    ...
                    if (result) {
                      ...
                      if(check === true){
                          ...
                          console.log("最終金額"+final);
                          console.log('是否更新'+user_final_money);
                          ...
                          console.log("檢查是否持有該股票"+stock_name);
                          try{
                                const dbRef = ref(getDatabase());
                                const dbRef2 = child(dbRef,"Buy/"+account);
            ...
            <body>
                <div class='warp'>
                    ...
                    <div class='rightBuy'>
                        ...
                        <div class='search1'>
                        <input class="search-bar1" type="text" name="search" id="search" placeholder="搜尋股票代號" onChange={(e)=>{setStocknum(e.target.value)}}/>
                        <button class="search-btn1" onClick={()=>{getStockprice(Stocknum)}}><img src={searchicon}/></button>
                        ...
                        {messages.map(message => 
                            <Message  idname={message.idname} stock_name={message.stock_name}  stock_price={message.stock_price} time={message.time}></Message>
                        )}
                        </div>
